 ...
 **      cases==4: The MAP method to derive decision rule
 ...
 ** Modified by
 **   - Niloo Ranjan
 **   - 09/15/2015: added fourth case for deriving decision rule using
 **                  likelihood ratio
 **   - 10/07/2015: replaced the previous fouth case with forth case for 
 **                 classification using MAP method
 **
 ************************************************************************/
        ...
        case 4:
          //  cout << "comes here " << endl;
            for (i=0; i<c; i++) {
                ...
                disc(0,i) = (1.0/((pow(2*PI, 0.5*nf))*(sqrt(detCovar))))*(exp(ret))*Pw(i,0);
